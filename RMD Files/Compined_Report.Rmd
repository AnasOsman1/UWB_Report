...
Finally, the Position Dilution of Precision (PDOP) Analysis is done to aid in the accuracy assessment of each area in Figure 5 to bring the experiments to a conclusion. We may deduce from the PDOP data that the Exp1 Area 2 setup has the lowest error in localization measurement. While Area 3 is the best configuration for Exp2 in terms of coverage and error, Area 2 is the best configuration for Exp3 in terms of coverage and error. Finally, we can infer from all of the provided findings that the ideal configuration for anchor placement is seen at Exp2/ Area3, where we are guaranteed both highest coverage Area and lowest error.
